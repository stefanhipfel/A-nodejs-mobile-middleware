	...
	if(completedTasks === tasks.length) {
		...
		if(finalTask) {
			...
			for (var index in mainCategoryList ) {
				...
				for(var products in mainCategoryList[index].items) {
					mainCategoryList[index].items[products].items.save(function() {
				...
				self._callSoapServices('categoryList', args, function(err, data) {
					...
					if(!data.return.categoriesContainer.categories) {
						...
						if(categories.domainName) {
							...
							mainCategoryList[categories.domainName].items.push(new MongoDB.subcategories({name: categories.name, leaf: true}));
						} 
					...
					if (checkIfTasksComplete(false, self) === true) {
						callback(null, 'ok');
					};
				});
			}
			...
			var  task = (function(args) {
				return function() {
					self._callSoapServices('productList', args, function(err, data) {
						...
						if (checkIfTasksComplete(true, self) === true) {
							callback(null, 'ok')
						};
					});
				}
